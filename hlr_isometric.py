...
def compute_hlr_isometric(shape):
    ...
    """
    # Изометрический вид "сверху-сзади-справа"
    # Направление взгляда: (1, 1, 1) - верх/сзади/право
    view_dir = gp_Dir(1.0, 1.0, 1.0)
    
    # Точка наблюдения (далеко от объекта)
    eye_point = gp_Pnt(1000, 1000, 1000)
    
    # Задаём систему координат проектора
    # X direction = перпендикулярен view_dir и лежит в плоскости XY
    x_dir = gp_Dir(-1.0, 1.0, 0.0)  # (-1, 1, 0) - перпендикулярен (1, 1, 1) в плоскости XY
...
def project_3d_to_2d_iso(x, y, z):
    """
    Проецирует 3D точку на 2D плоскость изометрии (вид сверху-сзади-справа)
    Z модели = визуально сверху
    """
    # Изометрическая проекция: сверху-сзади-справа
    # Камера смотрит в направлении (1, 1, 1)
    # Z модели должен быть визуально сверху (ось Y на 2D)
    ...
    # Ось X 2D = диагональ XY в 3D (слева-направо)
    # Ось Y 2D = ось Z в 3D (снизу-вверх) + перспектива XY
    px = (x - y) * cos_a
    py = z * cos_a + (x + y) * sin_a
